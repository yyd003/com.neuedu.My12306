...
public class LoginServlet extends HttpServlet {
	...
	private void doLogin(HttpServletRequest request,
		...
		// 取得请求参数
		User user = new User();
		// 填充用户，以后在别的页面还要显示这个用户的相关信息，所以要保存到session中
		populate(request, user);
		...
		// 检查用户是否存在,dbuser的信息内容最完整，除了ＩＰ要修改，其它都有了，这个要保存至session
		User dbUser = userService.login(user.getUsername(),
				Md5Utils.md5(user.getPassword()));
		// 如果没有查找到此用户，则登录失败，并将出错信息写入msg，流程重新跳回/Login.jsp
		if (dbUser == null) {
			msg = "用户名或密码错误";
			request.setAttribute("msg", msg);
			request.getRequestDispatcher("/Login.jsp").forward(request,
			...
			// 如果查找到此用户，代表登录成功，需要改写用户的部分登录信息，如ip,code,autoLogin
			dbUser.setLoginIp(user.getLoginIp());
			dbUser.setAutoLogin(user.getAutoLogin());
			dbUser.setCode(user.getCode());
			// 将用户写进session,以便在其它页面应用其信息
			se.setAttribute("user", dbUser);
			...
			// 根据其rule值（管理员rule=1,普通用户rule=2）
			// 　做相应跳转
			if ("1".equals(dbUser.getRule())) {
				// request.getContextPath()取项目上下文路径，以保证和项目发
				// 布路径一致
				// response.sendRedirect(request.getContextPath()
				// + "/Admin/Index.jsp");
				response.sendRedirect(request.getContextPath()
